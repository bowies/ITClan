...
<th colspan="2">이름  *</th>
<td colspan="4">${personalmemberdto.name }&nbsp; ${2016-(fn:substring(personalmemberdto.birth,0,4)) }세 / ${personalmemberdto.memberID} </td>
</tr>
...
<c:otherwise>
<c:forEach items="${portfoliodto}" var="pfdto">
<a href="javascript:downport('${pfdto.portfolioNum}')">${pfdto.portfolioName}</a><br>
</c:forEach>
</c:otherwise>
</c:choose>
</div>
...
<c:otherwise>
<c:forEach items="${resumedto }" var="resumedto">
<a href="javascript:downresume('${resumedto.resumeNum}')">${resumedto.resumeName }</a><br>
</c:forEach>
</c:otherwise>
</c:choose>
</div>
